...
//get all *.txt files
let files = fs.readdirSync('.').filter(file => file.endsWith('.txt'));
//for each file

for (let i = 0; i < files.length; i++) {
    const file_path = files[i];
    //outpath is file_path but with .html
    const outpath = file_path.substring(0, file_path.length - 4) + '.html';
    ...
    function punctuate(line) {
        ...
        line = line.replace(/。/g, '<span class="punctuation">。</span>');
    ...
    var HTML_DEF = `<span class="char" onclick="">`;
        ...
        HTML_DEF += `\t\t<span class="display_c"><span class="display_cr" onclick="">${char}</span><span class="cursive" onclick="">${char}</span></span>\n`;
        ...
        const simplified = [4,5,6,7,11,14,15];
        ...
        const s_indexes = [1,5,6,7,14,15,13,8,9];

        HTML_DEF += `</span>`;
        HTML_DEF += `<span class="cursivetooltip">`;
        for (var s_i = 0; s_i < s_indexes.length; s_i++) {
            var s = s_indexes[s_i];
            var this_char = char;
            //if char is traditional, and we're on a simplified row, replace
            if (simplified.indexOf(s) >=0) {
                var T_Index = T.indexOf(char);
                if (T_Index >= 0){
                    this_char = S[T_Index];
                    console.log(`for font ${s_i} I'm replacing traditional character ${char} with simplified ${this_char}`);
                }
            }
            HTML_DEF += `<span class="s${s} tooltipsinglechar">${this_char}</span>`;
            if (s_i>0 && (s_i+1)%3==0) {
                HTML_DEF += `<br/>`;
            }
        }
    ...
    */
    ...
    //write to disk
    fs.writeFileSync(outpath, HTML_FILE);
}